...
// ═══════════════════════════════════════
// CLOUDFLARE CHALLENGE BEKLEYİCİ
// ═══════════════════════════════════════

/**
 * Sayfa Cloudflare challenge sayfasındaysa otomatik çözülmesini bekler.
 * Cloudflare'in standart JS challenge'ı (~5sn) stealthed browser'da otomatik geçer.
 * Eğer geçmezse hata fırlatır (retry mekanizması yakalanır).
 */
export async function waitThroughCloudflare(page, timeout = 25000) {
  const CF_TITLE_PATTERNS = ['Just a moment', 'Attention Required', 'Please Wait', 'DDoS protection'];

  const title = await page.title();
  const isCfChallenge = CF_TITLE_PATTERNS.some(p => title.includes(p));
  if (!isCfChallenge) return; // Challenge yok, devam et

  // Challenge var — sayfanın başlığı değişene kadar bekle (redirect = gerçek sayfa)
  const deadline = Date.now() + timeout;
  while (Date.now() < deadline) {
    await sleep(1000);
    const currentTitle = await page.title();
    const stillCf = CF_TITLE_PATTERNS.some(p => currentTitle.includes(p));
    if (!stillCf) return; // Challenge geçti
  }

  throw new Error('Cloudflare challenge otomatik çözülemedi (bot koruması manuel müdahale gerektiriyor)');
}
...
async function singleMeasurement(browser, url, coldCache, sharedContext = null) {
  ...
  try {
    ...
    // Gerçek tarayıcı başlıkları — eksik sec-fetch-* ve sec-ch-ua başlıkları 403'e neden olabilir
    ...
    // Ek bot-tespiti engelleme (stealth plugin'e ek olarak)
    ...
    await waitThroughCloudflare(page); // Challenge sayfasındaysa geçmesini bekle
    await sleep(500);
