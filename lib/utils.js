...
/**
 * İsim formatlama — Türkçe karakter desteğiyle
 * İlk kelime(ler): Baş harfi büyük, geri kalanı küçük
 * Son kelime (soyad): Tamamı büyük harf
 * Örnek: "emirkan dağ" → "Emirkan DAĞ"
 *        "ahmet mehmet öztürk" → "Ahmet Mehmet ÖZTÜRK"
 */
...
/**
 * Manuel GreenIT verilerinden sentetik ölçüm dizisi üretir.
 * Her satır aynı değerleri taşır (kullanıcının girdiği ortalamalar).
 * CO2 ve su EcoIndex'ten hesaplanır.
 */
export function buildManualGreenITResults(count, { ecoIndex, requestCount, pageSizeKB, domSize }) {
...
// ═══════════════════════════════════════
// DURUM BELİRLEME FONKSİYONLARI
// GreenIT Analysis quantile tabloları (quintile sınırları)
// Kaynak: EcoIndex quantile dizileri — %20, %40, %60, %80 yüzdelikler
// ═══════════════════════════════════════

/**
 * Sayfa ağırlığı durumu (KB cinsinden girdi)
 * Quintile sınırları QUANTILES_SIZE dizisinden: [4]=479, [8]=1099, [12]=1876, [16]=3402
 */
export function getPageSizeDurum(sizeKB) {
  if (sizeKB === null || sizeKB === undefined || isNaN(sizeKB)) return '-';
  if (sizeKB <= 479)  return 'Çok hafif';
  if (sizeKB <= 1099) return 'Hafif';
  if (sizeKB <= 1876) return 'Ortalama';
  if (sizeKB <= 3402) return 'Ağır';
  return 'Çok ağır';
}

/**
 * DOM karmaşıklık durumu
 * Quintile sınırları QUANTILES_DOM dizisinden: [4]=233, [8]=476, [12]=843, [16]=1459
 */
export function getDomDurum(domSize) {
  if (domSize === null || domSize === undefined || isNaN(domSize)) return '-';
  if (domSize <= 233)  return 'Çok basit';
  if (domSize <= 476)  return 'Basit';
  if (domSize <= 843)  return 'Ortalama';
  if (domSize <= 1459) return 'Karmaşık';
  return 'Çok Karmaşık';
}

/**
 * HTTP istek sayısı durumu
 * Quintile sınırları QUANTILES_REQ dizisinden: [4]=34, [8]=63, [12]=105, [16]=170
 */
export function getRequestDurum(reqCount) {
  if (reqCount === null || reqCount === undefined || isNaN(reqCount)) return '-';
  if (reqCount <= 34)  return 'Çok az';
  if (reqCount <= 63)  return 'Az';
  if (reqCount <= 105) return 'Ortalama';
  if (reqCount <= 170) return 'Çok';
  return 'Aşırı';
}
