...
export async function createDocxReport(outputPath, institutionName, url, coldResults, warmResults, lhResults, screenshotPath, authorName, advisorName, reportDate) {
  ...
  // ═══════════════════════════════════════
  // 6b. BÖLÜM 7 BAŞLIKLARINI DÜZELT
  // ═══════════════════════════════════════
  // Template'de 7.4-7.7 hepsi "Provide print stylesheet" olarak tekrarlanmış
  docXml = docXml.replace(/<w:t>7\.4[^<]*Provide print stylesheet<\/w:t>/g, '<w:t>7.4 Optimize render-blocking resources</w:t>');
  docXml = docXml.replace(/<w:t>7\.5[^<]*Provide print stylesheet<\/w:t>/g, '<w:t>7.5 Reduce HTTP requests</w:t>');
  docXml = docXml.replace(/<w:t>7\.6[^<]*Provide print stylesheet<\/w:t>/g, '<w:t>7.6 Improve caching strategy</w:t>');
  docXml = docXml.replace(/<w:t>7\.7[^<]*Provide print stylesheet<\/w:t>/g, '<w:t>7.7 Simplify DOM structure</w:t>');
  // Ayrıca başlık metninin farklı formatta olabileceği ihtimaline karşı
  docXml = docXml.replace(/<w:t[^>]*>7\.4\s+Provide print stylesheet<\/w:t>/g, '<w:t>7.4 Optimize render-blocking resources</w:t>');
  docXml = docXml.replace(/<w:t[^>]*>7\.5\s+Provide print stylesheet<\/w:t>/g, '<w:t>7.5 Reduce HTTP requests</w:t>');
  docXml = docXml.replace(/<w:t[^>]*>7\.6\s+Provide print stylesheet<\/w:t>/g, '<w:t>7.6 Improve caching strategy</w:t>');
  docXml = docXml.replace(/<w:t[^>]*>7\.7\s+Provide print stylesheet<\/w:t>/g, '<w:t>7.7 Simplify DOM structure</w:t>');
  ...
  while ((match = tblRegex.exec(xml)) !== null) {
    ...
    if (rowCount === 5) {
      if (smallTableIdx === tableIndex) {
        ...
        newTbl = replaceNthCellContent(newTbl, 2, 1, buildSkorXml(avgPageSizeMB));  // Sayfa Ağırlığı (MB)
        newTbl = replaceNthCellContent(newTbl, 3, 1, buildSkorXml(avgDom));          // Karmaşıklık
        newTbl = replaceNthCellContent(newTbl, 4, 1, buildSkorXml(avgReq));          // İstekler

        // ──── DURUM sütunu (Cell 4) — otomatik durum ────
        newTbl = replaceNthCellContent(newTbl, 2, 4, buildDurumXml(pageSizeStatuses, durumPageSize));   // Sayfa Ağırlığı durumu
        newTbl = replaceNthCellContent(newTbl, 3, 4, buildDurumXml(domStatuses, durumDom));        // Karmaşıklık durumu
        newTbl = replaceNthCellContent(newTbl, 4, 4, buildDurumXml(reqStatuses, durumReq));        // İstekler durumu
...
function rebuildDataTable(tableXml, results, colWidths, type) {
  ...
  // Lighthouse tablosunda ikinci FCP'yi SI olarak düzelt
  if (type === 'lighthouse') {
    const lastFcpIdx = headerPart.lastIndexOf('<w:t>FCP</w:t>');
    const firstFcpIdx = headerPart.indexOf('<w:t>FCP</w:t>');
    if (lastFcpIdx >= 0 && firstFcpIdx !== lastFcpIdx) {
      headerPart = headerPart.substring(0, lastFcpIdx) + '<w:t>SI</w:t>' + headerPart.substring(lastFcpIdx + '<w:t>FCP</w:t>'.length);
    }
  }
...
function replaceSectionText(xml, sectionId, newText, coldGrade = null, warmGrade = null, coldResults = null, warmResults = null) {
  ...
  if (sectionId === 'ÖZET' && coldGrade && warmGrade) {
    ...
    // Sayfa sonu: Cold Cache'den sonra, Warm Cache'den önce
    newContent += `
    <w:p w14:paraId="${randomParaId()}" w14:textId="${randomParaId()}" w:rsidR="00530778" w:rsidRDefault="00530778">
      <w:pPr><w:spacing w:after="0"/></w:pPr>
      <w:r><w:br w:type="page"/></w:r>
    ...
    // ─── Warm Cache Sonucu ───
    newContent += getGradeDrawingXml(warmGrade, 'Warm Cache Sonucu');
  }
